...
public class ProductRedisTest {
    ...
    public void findAll_populatesCache() {
        ...
        Object cached = productsCache
                .get(new CompositeCacheKey(0, 20), k -> null)
                .await()
                .indefinitely();
    ...
    public void cacheInvalidatedAfterCreate() {
        ...
        Product product = new Product(
                "New Product", "Description", new Money(new BigDecimal("100.00"), "BRL"), 5, newCategoryId());
        given().body(product)
        ...
        assertThrows(
                ...
                () -> productsCache
                        .get(productsCache.getDefaultKey(), k -> null)
                        .await()
                        .indefinitely());
        assertThrows(
                IllegalArgumentException.class,
                () -> categoryCache
                        .get(categoryCache.getDefaultKey(), k -> null)
                        .await()
                        .indefinitely());
        assertThrows(
                IllegalArgumentException.class,
                () -> activeCache
                        .get(activeCache.getDefaultKey(), k -> null)
                        .await()
                        .indefinitely());
    }
    ...
    public void cacheInvalidatedAfterUpdate() {
        Product product = new Product(
                "Original Product", "Description", new Money(new BigDecimal("100.00"), "BRL"), 5, newCategoryId());
    ...
    public void cacheInvalidatedAfterDelete() {
        Product product = new Product(
                "Delete Product", "Description", new Money(new BigDecimal("100.00"), "BRL"), 5, newCategoryId());
        String id = given().body(product)
                .contentType(ContentType.JSON)
                .when()
                .post("/products")
